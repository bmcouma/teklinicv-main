...
We want documentation at `docs.teklinicv.com`, a proper website with navigation, search, theming, and interactive features.
...
3. A domain (e.g. `docs.teklinicv.com`) pointing to that server
...
**The solution:** [MkDocs](https://github.com/mkdocs/mkdocs) with [Material theme](https://github.com/squidfunk/mkdocs-material). You write Markdown in `docs/`, MkDocs generates HTML/CSS/JavaScript, and GitHub Pages hosts it at `docs.teklinicv.com`.
...
## Configuration: [`mkdocs.yaml`](https://github.com/teklinicv/teklinicv/blob/main/mkdocs.yaml)
...
Generates the API reference from Python docstrings. The entire [API Reference](../api_reference/index.md) section is auto-generated from `src/teklinicv/`.

### [`mkdocs-macros-plugin`](https://mkdocs-macros-plugin.readthedocs.io/): Dynamic Content

Lets you inject code-generated values into Markdown. [`docs/docs_templating.py`](https://github.com/teklinicv/teklinicv/blob/main/docs/docs_templating.py) runs during the build. It imports values directly from TekliniCV's code and exposes them as variables. It's heavily used in [YAML Input Structure: `cv` Field](../user_guide/yaml_input_structure/cv.md) page.
...
These are auto-generated PNG images. Run `just update-entry-figures` to regenerate them from [`docs/user_guide/sample_entries.yaml`](https://github.com/teklinicv/teklinicv/blob/main/docs/user_guide/sample_entries.yaml).
...
**The workflow** ([`.github/workflows/deploy-docs.yaml`](https://github.com/teklinicv/teklinicv/blob/main/.github/workflows/deploy-docs.yaml)):
...
3. **Deploy step:**
      ...
      - Makes it available at `docs.teklinicv.com`
