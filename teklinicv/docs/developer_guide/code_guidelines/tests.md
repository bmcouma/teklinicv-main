...
Each test file tests all classes and functions in its corresponding source file. The structure mirrors `src/teklinicv/`:

```
src/teklinicv/renderer/templater/date.py
    → tests/renderer/templater/test_date.py
    (tests all functions and classes in date.py)

src/teklinicv/schema/models/cv/section.py
