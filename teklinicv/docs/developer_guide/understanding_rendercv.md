...
# Understanding TekliniCV

This guide walks you through how TekliniCV works, explaining each step and the tools we use.

## The Core Workflow

TekliniCV does more than this (Markdown, HTML, PNG outputs, watching files, etc.), but at its core, what happens is:
...
TekliniCV generates a Typst file from your YAML and compiles it with the Typst compiler to produce your CV as a PDF.
...
`ruamel.yaml` is being called in [`src/teklinicv/schema/yaml_reader.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/schema/yaml_reader.py).
...
**TekliniCV's entire data model is `pydantic` models all the way down:**

```python
class TekliniCVModel(BaseModel):
...
Each field is another `pydantic` model. `Cv` contains more `pydantic` models like `EducationEntry`, `ExperienceEntry`, etc. It's nested validation: when you validate `TekliniCVModel`, `pydantic` automatically validates every nested model too. One `model_validate()` call checks the entire structure.

See [`src/teklinicv/schema/models/teklinicv_model.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/schema/models/teklinicv_model.py) for the top-level model.
...
Typst templates live in [`src/teklinicv/renderer/templater/templates/typst/`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/renderer/templater/templates/typst/).

`jinja2` is being called in [`src/teklinicv/renderer/templater/templater.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/renderer/templater/templater.py).
...
See [`src/teklinicv/renderer/templater/markdown_parser.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/renderer/templater/markdown_parser.py). The `markdown_to_typst()` function does this conversion.
...
`typst` is being called in [`src/teklinicv/renderer/pdf_png.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/renderer/pdf_png.py).

## The Complete Pipeline

When you run `teklinicv render cv.yaml`:

1. **Parse** - `ruamel.yaml` reads YAML → Python dict
2. **Validate** - `pydantic` validates dict → `TekliniCVModel` object
...
- [`src/teklinicv/cli/render_command/run_teklinicv.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/cli/render_command/run_teklinicv.py): The complete flow
- [`src/teklinicv/schema/models/teklinicv_model.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/schema/models/teklinicv_model.py): The top-level Pydantic model
- [`src/teklinicv/renderer/templater/templater.py`](https://github.com/teklinicv/teklinicv/blob/main/src/teklinicv/renderer/templater/templater.py): Template rendering
