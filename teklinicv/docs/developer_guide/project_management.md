...
When you look at TekliniCV's repository, you see:

```
.
├── src/                     ← The actual TekliniCV code
...
- Share TekliniCV with users (`pip install teklinicv`)
...
TekliniCV is a Python project. The actual source code lives in `src/teklinicv/`. Why do we need all these other files - `pyproject.toml`, `uv.lock`, `justfile`, `.github/workflows/`, etc.?
...
You could tell them "download these files, install dependencies with `pip install -r requirements.txt`, and run them with `python main.py`". But users want `pip install teklinicv` and have it work instantly with `teklinicv` command.
...
### [`pyproject.toml`](https://github.com/teklinicv/teklinicv/blob/main/pyproject.toml)
...
- Dependencies (what packages TekliniCV needs)
- Entry points (makes `teklinicv` a command)
- Build configuration (how to package TekliniCV)
- Tool settings (`ruff`, `ty`, `pytest`, etc.)

Open the file to see the full configuration with detailed comments.

### [`justfile`](https://github.com/teklinicv/teklinicv/blob/main/justfile)
...
### [`scripts/`](https://github.com/teklinicv/teklinicv/tree/main/scripts)

Python scripts that automate some repetitive tasks that are not part of TekliniCV's functionality.
...
- `update_examples.py`: Regenerate all example YAML files and PDFs in [`examples/`](https://github.com/teklinicv/teklinicv/tree/main/examples) folder
- `create_executable.py`: Build standalone executable of TekliniCV

These scripts are called by `just` commands (`just update-schema`, `just update-examples`, etc.).

### [`.pre-commit-config.yaml`](https://github.com/teklinicv/teklinicv/blob/main/.pre-commit-config.yaml)
...
### [`uv.lock`](https://github.com/teklinicv/teklinicv/blob/main/uv.lock)

A dependency lock file. This is a record of the exact version of every package TekliniCV uses (including dependencies of dependencies of dependencies...).
