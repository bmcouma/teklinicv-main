...
## Why Docker for TekliniCV?

TekliniCV installs easily with `pip install teklinicv` if you have Python. Most users don't need Docker.
...
The TekliniCV Docker image is a ready-made environment with Python and TekliniCV pre-installed. Just run:
```bash
...
When TekliniCV publishes a GitHub release, the [`release.yaml` workflow](https://github.com/teklinicv/teklinicv/blob/main/.github/workflows/release.yaml) automatically builds and publishes the TekliniCV image to GHCR at `ghcr.io/teklinicv/teklinicv`.

When users run `docker run ghcr.io/teklinicv/teklinicv`, Docker automatically pulls the image from the registry if it's not already on their machine.
