...
1. Add network name to `SocialNetworkName` type
    ...
    Edit `src/teklinicv/schema/models/cv/social_network.py`:
...
3. (Optional) Add username validation
    ...
    If your network has special username format requirements, edit `src/teklinicv/schema/models/cv/social_network.py`:
...
4. (Optional) Add custom URL generation
    ...
    If URL generation requires special logic (not just base + username), edit `src/teklinicv/schema/models/cv/social_network.py`:
...
5. Add Font Awesome icon
    ...
    Edit `src/teklinicv/renderer/templater/connections.py`:
