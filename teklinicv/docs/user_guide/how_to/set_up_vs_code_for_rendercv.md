# Set Up VS Code for TekliniCV

Visual Studio Code can be configured to provide a live preview environment for writing your CV with TekliniCV. This setup enables you to see your changes reflected in the PDF instantly as you type, making the CV editing process smooth and interactive.
...
2. **Run `teklinicv render` with the watch mode**:

    ```bash
    teklinicv render --watch John_Doe_CV.yaml
    ```

    The `--watch` flag tells TekliniCV to monitor the YAML file for changes and automatically re-render the PDF whenever the file is saved.

3. **Arrange your workspace**:

    - Place your YAML file on the left side of the editor
    - Open the generated PDF (from `teklinicv_output/`) on the right side

4. **Start editing**: As you make changes to the YAML file, they will be automatically saved, triggering TekliniCV to regenerate the PDF. The PDF viewer will update to show your changes in real-time.
...
- Verify that `teklinicv render --watch` is running in the terminal without errors
