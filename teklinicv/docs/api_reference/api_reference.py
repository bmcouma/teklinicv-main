...
src_teklinicv = repository_root / "src" / "teklinicv"
nav[("src", "teklinicv")] = "index.md"

# Process each Python file in the objects directory
for path in sorted(src_teklinicv.rglob("*.py")):
    # Skip __init__.py files and __main__.py files
    if path.name in ("__init__.py", "__main__.py"):
        continue

    # Get the relative path from the objects directory
    module_path = path.relative_to(src_teklinicv).with_suffix("")
    doc_path = module_path.with_suffix(".md")
    parts = (f"teklinicv.{module_path.parts[0]}", *module_path.parts[1:])
    ...
    with mkdocs_gen_files.open(f"api_reference/{doc_path}", "w") as fd:
        module_ident = "teklinicv." + ".".join(module_path.parts)
