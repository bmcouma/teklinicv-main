...
with tempfile.TemporaryDirectory() as temp_dir:
    ...
    # Copy teklinicv to temp directory
    shutil.copytree(root_path / "src" / "teklinicv", temp_path / "teklinicv")

    # Create entry point script
    teklinicv_file = temp_path / "teklinicv.py"
    teklinicv_file.write_text("import teklinicv.cli.app as app; app.app()")
    ...
    match sys.platform:
        case "win32":
            original_name = "teklinicv.exe"
            new_name = f"teklinicv-{platform_name}-{machine_name}.exe"
        case _:
            original_name = "teklinicv"
            new_name = f"teklinicv-{platform_name}-{machine_name}"
