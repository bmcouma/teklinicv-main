...
from teklinicv.schema.models.cv.section import available_entry_type_names


class TestCv:
    def test_teklinicv_sections(self, request: pytest.FixtureRequest):
        entry_type_names = [
            "".join(
        ...
        assert len(cv.teklinicv_sections) == len(available_entry_type_names)
        for section in cv.teklinicv_sections:
