...
from teklinicv.exception import TekliniCVUserError
from teklinicv.schema.models.teklinicv_model import TekliniCVModel
from teklinicv.schema.teklinicv_model_builder import (
    build_teklinicv_dictionary,
    build_teklinicv_dictionary_and_model,
    build_teklinicv_model_from_commented_map,
)
from teklinicv.schema.sample_generator import dictionary_to_yaml
...
class TestBuildRendercvDictionary:
    ...
    def test_basic_input_without_overlays(
        ...
        result = build_teklinicv_dictionary(yaml_input)
    ...
    def test_single_overlay(
        ...
        result = build_teklinicv_dictionary(main_yaml, **kwargs)  # pyright: ignore[reportArgumentType]
    ...
    def test_all_overlays_simultaneously(self, create_yaml_file_fixture):
        ...
        result = build_teklinicv_dictionary(
            main_file,
    ...
    def test_render_command_single_override(
        ...
        result = build_teklinicv_dictionary(yaml_input, **kwargs)
    ...
    def test_render_command_preserves_existing_settings(self):
        ...
        result = build_teklinicv_dictionary(yaml_input, typst_path="new.typ")
    ...
    def test_overrides_parameter(self, minimal_input_dict, overrides, expected_checks):
        ...
        result = build_teklinicv_dictionary(yaml_input, overrides=overrides)
...
class TestBuildRendercvModelFromDictionary:
    def test_basic_model_creation_without_optionals(self, minimal_input_dict):
        ...
        assert isinstance(model, TekliniCVModel)
    ...
    def test_with_input_file_path(self, minimal_input_dict, tmp_path):
        ...
        model = build_teklinicv_model_from_commented_map(
    ...
    def test_validation_context_current_date(self, minimal_input_dict, settings):
        ...
        model = build_teklinicv_model_from_commented_map(input_dict)
    ...
    def test_validation_context_with_input_file_path(
        ...
        model = build_teklinicv_model_from_commented_map(input_dict, input_file_path)  # pyright: ignore[reportArgumentType]
...
class TestBuildRendercvModel:
    ...
    def test_basic_model_creation(
        ...
        _, model = build_teklinicv_dictionary_and_model(yaml_input)
    ...
    def test_with_single_overlay(
        ...
        kwargs = {f"{overlay_key}_file_path_or_contents": overlay_input}
        _, model = build_teklinicv_dictionary_and_model(main_yaml, **kwargs)  # pyright: ignore[reportArgumentType]
    ...
    def test_with_all_overlays(self, create_yaml_file_fixture):
        ...
        _, model = build_teklinicv_dictionary_and_model(
            main_file,
            design_file_path_or_contents=design_file,
            locale_file_path_or_contents=locale_file,
        )
    ...
    def test_with_render_command_overrides(self, minimal_input_dict, overrides):
        ...
        _, model = build_teklinicv_dictionary_and_model(main_yaml, **overrides)
    ...
    def test_with_overrides_parameter(
        ...
        _, model = build_teklinicv_dictionary_and_model(yaml_input, overrides=overrides)

        assert isinstance(model, TekliniCVModel)
        assert model.cv.name == expected_name

    def test_with_fixture_input_file(self, input_file_path):
        _, model = build_teklinicv_dictionary_and_model(input_file_path)
        assert isinstance(model, TekliniCVModel)
    ...
    def test_with_yaml_string_using_ruamel(self):
        ...
        _, model = build_teklinicv_dictionary_and_model(yaml_string)
        assert isinstance(model, TekliniCVModel)
    ...
    def test_invalid_file_extension_raises_error(self, tmp_path):
        ...
        with pytest.raises(TekliniCVUserError):
            build_teklinicv_dictionary_and_model(invalid_file_path)

    def test_nonexistent_file_raises_error(self, tmp_path):
        non_existent_file_path = tmp_path / "non_existent_file.yaml"

        with pytest.raises(TekliniCVUserError):
            build_teklinicv_dictionary_and_model(non_existent_file_path)
    ...
    def test_design_file_overlay_loads_and_applies(self, create_yaml_file_fixture):
        ...
        dictionary, model = build_teklinicv_dictionary_and_model(
            main_file,
            design_file_path_or_contents=design_file,
        )
    ...
    def test_locale_file_overlay_loads_and_applies(self, create_yaml_file_fixture):
        ...
        dictionary, _ = build_teklinicv_dictionary_and_model(
            main_file,
            locale_file_path_or_contents=locale_file,
        )
