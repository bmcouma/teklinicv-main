...
from teklinicv.schema.sample_generator import (
    create_sample_teklinicv_pydantic_model,
...
class TestCreateSampleRendercvPydanticModel:
    ...
    def test_creates_valid_model_for_all_themes_and_locales(self, theme, locale):
        data_model = create_sample_teklinicv_pydantic_model(
            name="John Doe", theme=theme, locale=locale
        )
        assert isinstance(data_model, TekliniCVModel)
    ...
    def test_creates_model_with_unicode_name(self):
        name = "Matías"
        data_model = create_sample_teklinicv_pydantic_model(name=name)
