...
from teklinicv.renderer.templater.connections import (
    compute_connections,
...
from teklinicv.schema.models.cv.custom_connection import CustomConnection
from teklinicv.schema.models.cv.cv import Cv
from teklinicv.schema.models.cv.social_network import SocialNetwork, SocialNetworkName
from teklinicv.schema.models.design.classic_theme import (
    ClassicTheme,
    Connections,
    Header,
)
from teklinicv.schema.models.locale.locale import EnglishLocale
...
def create_teklinicv_model(
    ...
    *,
    ...
    """Create a test TekliniCVModel with specified connection formatting options.

    Used to test different connection rendering configurations (icons, links, phone format).
    """
    return TekliniCVModel(
        cv=cv,
        design=ClassicTheme(
            header=Header(
...
class TestParseConnections:
    ...
    def test_parse_single_field_type(self, field, value, expected_count, expected_icon):
        ...
        model = create_teklinicv_model(cv)
    ...
    def test_phone_formatting(self, phone, phone_format, expected_body):
        ...
        model = create_teklinicv_model(cv, phone_format=phone_format)
...
class TestComputeConnectionsForTypst:
    ...
    def test_icon_and_link_combinations(self, use_icons, make_links, expected_patterns):
        cv = create_cv(key_order=["email"], email="john@example.com")
        model = create_teklinicv_model(cv, use_icons=use_icons, make_links=make_links)
    ...
    def test_connection_without_url_has_no_link(self):
        cv = create_cv(key_order=["location"], location="New York, NY")
        model = create_teklinicv_model(cv, use_icons=True, make_links=True)
    ...
    def test_plain_text_output_when_no_formatting(self):
        cv = create_cv(key_order=["email"], email="john@example.com")
        model = create_teklinicv_model(cv, use_icons=False, make_links=False)
