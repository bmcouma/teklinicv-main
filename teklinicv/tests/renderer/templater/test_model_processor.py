...
from teklinicv.renderer.templater.model_processor import process_fields, process_model
...
def model(request: pytest.FixtureRequest) -> TekliniCVModel:
    """Return a test TekliniCVModel with keywords either set or empty.
    ...
    teklinicv_model = TekliniCVModel(cv=cv)
    teklinicv_model.settings.current_date = Date(2024, 2, 1)
    teklinicv_model.settings.bold_keywords = request.param
...
class TestProcessModel:
    def test_markdown_output_has_correct_structure(self, model):
        ...
        entry = result.cv.teklinicv_sections[0].entries[0]
    ...
    def test_handles_cv_with_no_sections(self):
        ...
        result = process_model(teklinicv_model, "markdown")
