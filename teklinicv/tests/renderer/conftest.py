...
from teklinicv.schema.models.cv.cv import Cv
from teklinicv.schema.models.cv.entries.bullet import BulletEntry
from teklinicv.schema.models.cv.entries.education import EducationEntry
from teklinicv.schema.models.cv.entries.experience import ExperienceEntry
from teklinicv.schema.models.cv.entries.normal import NormalEntry
from teklinicv.schema.models.cv.entries.numbered import NumberedEntry
from teklinicv.schema.models.cv.entries.one_line import OneLineEntry
from teklinicv.schema.models.cv.entries.publication import PublicationEntry
from teklinicv.schema.models.cv.entries.reversed_numbered import ReversedNumberedEntry
from teklinicv.schema.models.cv.social_network import SocialNetwork
...
@pytest.fixture
def minimal_teklinicv_model() -> TekliniCVModel:
    """Create a minimal TekliniCVModel for testing.

    Returns:
        A TekliniCVModel with minimal CV data (name + one section).
    """
    cv = Cv(
        name="John Doe",
        sections={
    ...
    return TekliniCVModel(cv=cv, settings=Settings(current_date=Date(2025, 11, 30)))


@pytest.fixture
def full_teklinicv_model(testdata_dir: pathlib.Path) -> TekliniCVModel:
    """Create a comprehensive TekliniCVModel with all entry combinations.
    ...
    Returns:
        A TekliniCVModel with ~400 entry instances across all entry types.
    """
