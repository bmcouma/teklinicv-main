...
from teklinicv.cli.render_command.progress_panel import CompletedStep, ProgressPanel
from teklinicv.exception import TekliniCVUserError, TekliniCVValidationError
...
class TestProgressPanelPrintUserError:
    def test_exits_with_code_1(self):
        panel = ProgressPanel(quiet=True)
        error = TekliniCVUserError(message="Test error message")
    ...
    def test_handles_error_without_message(self):
        panel = ProgressPanel(quiet=True)
        error = TekliniCVUserError(message=None)
...
class TestProgressPanelPrintValidationErrors:
    def test_exits_with_code_1(self):
        panel = ProgressPanel(quiet=True)
        errors: list[TekliniCVValidationError] = [
            TekliniCVValidationError(
                location=("cv", "name"),
