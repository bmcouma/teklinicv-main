...
from .run_teklinicv import run_teklinicv
...
@app.command(
    name="render",
    help=(
        "Render a YAML input file. Example: [yellow]teklinicv render"
        " John_Doe_CV.yaml[/yellow]. Details: [cyan]teklinicv render --help[/cyan]"
    ),
...
def cli_command_render(
    ...
    watch: Annotated[
        ...
        typer.Option(
            ...
            "-w",
            help=(
                "If provided, TekliniCV will automatically re-run when the input file is"
                " updated."
            ),
        ),
    ] = None,
    quiet: Annotated[
        ...
        typer.Option(
            "--quiet",
            "-q",
            help="If provided, TekliniCV will not print any messages.",
        ),
    ] = False,
    ...
    with ProgressPanel(quiet=quiet) as progress_panel:
        if watch:
            run_function_if_file_changes(
                input_file_path,
                lambda: run_teklinicv(input_file_path, progress_panel, **arguments),
            )
        else:
            run_teklinicv(input_file_path, progress_panel, **arguments)
