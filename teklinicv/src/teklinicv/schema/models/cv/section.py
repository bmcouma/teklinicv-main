...
class BaseTekliniCVSection(BaseModelWithoutExtraKeys):
    title: str
    entry_type: str
    entries: list[Any]
...
def create_section_models(
    ...
    return pydantic.create_model(
        ...
        __base__=BaseTekliniCVSection,
    )


section_models: dict[type[EntryModel] | type[str], type[BaseTekliniCVSection]] = {
...
) -> tuple[str, type[BaseTekliniCVSection]]:
...
def validate_section(sections_input: Any) -> Any:
    ...
    if isinstance(sections_input, list):
        ...
        if entry_type_name is None or section_type is None:
            raise pydantic_core.PydanticCustomError(
                CustomPydanticErrorTypes.other.value,
                "TekliniCV couldn't match this section with any entry types. Please"
        ...
        try:
            ...
            new_error = pydantic_core.PydanticCustomError(
                ...
                "There are problems with the entries. TekliniCV detected the entry type"
...
def get_teklinicv_sections(
    sections: dict[str, list[Any]] | None,
) -> list[BaseTekliniCVSection]:
    ...
    """
    sections_teklinicv: list[BaseTekliniCVSection] = []
    ...
    if sections is not None:
        for title, entries in sections.items():
            ...
            section = BaseTekliniCVSection(
                title=formatted_title,
                entry_type=entry_type_name,
                entries=entries,
            )
            sections_teklinicv.append(section)

    return sections_teklinicv
