...
from teklinicv.schema.models.design.classic_theme import Templates
from teklinicv.schema.models.locale.locale import Locale
    ...
    *,
    ...
    if "HIGHLIGHTS" in entry_fields:
        ...
        if highlights is None:
            raise TekliniCVInternalError("HIGHLIGHTS in fields but highlights is None")
        entry_fields["HIGHLIGHTS"] = process_highlights(highlights)

    if "AUTHORS" in entry_fields:
        authors = getattr(entry, "authors", None)
        if authors is None:
            raise TekliniCVInternalError("AUTHORS in fields but authors is None")
    ...
    if "START_DATE" in entry_fields:
        ...
        if start_date is None:
            raise TekliniCVInternalError("START_DATE in fields but start_date is None")
    ...
    if "END_DATE" in entry_fields:
        end_date = getattr(entry, "end_date", None)
        if end_date is None:
            raise TekliniCVInternalError("END_DATE in fields but end_date is None")
...
def process_date(
    *,
    ...
    raise TekliniCVInternalError("Date is not provided for this entry.")
...
def process_url(entry: Entry) -> str:
    ...
    raise TekliniCVInternalError("URL is not provided for this entry.")
...
def process_doi(entry: Entry) -> str:
    ...
    raise TekliniCVInternalError("DOI is not provided for this entry.")
