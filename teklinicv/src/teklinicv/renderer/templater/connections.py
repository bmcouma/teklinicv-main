...
from teklinicv.exception import TekliniCVInternalError
...
def compute_connections(
    ...
    Args:
        teklinicv_model: CV model with contact information.
...
def parse_connections(teklinicv_model: TekliniCVModel) -> list[Connection]:
    ...
    for key in teklinicv_model.cv._key_order:
        match key:
            case "email":
                emails = teklinicv_model.cv.email
            ...
            case "phone":
                phones = teklinicv_model.cv.phone
                if phones is None:
                    raise TekliniCVInternalError("phone key present but value is None")
                ...
                for phone in phones:
                    ...
                    body = phonenumbers.format_number(
                        ...
                        getattr(
                            ...
                            teklinicv_model.design.header.connections.phone_number_format.upper(),
                        ),
                    )
                    connections.append(
                        Connection(
            ...
            case "website":
                websites = teklinicv_model.cv.website
                if not websites:
                    raise TekliniCVInternalError("website key present but value is None")
            ...
            case "location":
                url = None
                body = str(teklinicv_model.cv.location)
                connections.append(
                    Connection(
            ...
            case "social_networks":
                if teklinicv_model.cv.social_networks is None:
                    raise TekliniCVInternalError(
                        "social_networks key present but value is None"
                    )
                for social_network in teklinicv_model.cv.social_networks:
                    url = social_network.url
                    if teklinicv_model.design.header.connections.display_urls_instead_of_usernames:
                        body = clean_url(url)
                    else:
            ...
            case "custom_connections":
                if teklinicv_model.cv.custom_connections is None:
                    raise TekliniCVInternalError(
                        "custom_connections key present but value is None"
                    )
                for custom_connection in teklinicv_model.cv.custom_connections:
                    url = (
...
def compute_connections_for_typst(teklinicv_model: TekliniCVModel) -> list[str]:
    ...
    Args:
        teklinicv_model: CV model with contact information and design settings.

    Returns:
        List of Typst-formatted connection strings ready for template insertion.
    """
    connections = parse_connections(teklinicv_model)

    show_icon = teklinicv_model.design.header.connections.show_icons
    hyperlink = teklinicv_model.design.header.connections.hyperlink
...
def compute_connections_for_markdown(teklinicv_model: TekliniCVModel) -> list[str]:
